...
export async function GET(
  ...
  try {
    ...
    const { eventId } = z.object({ eventId: z.uuid() }).parse(await context.params);
    return operationsResponse(await operationsApi.auditEvent(eventId, token));
  } catch (error) {
