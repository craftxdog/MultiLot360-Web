...
export async function DELETE(request: Request, context: { params: Promise<{ notificationId: string }> }) {
  try {
    assertOperationsOrigin(request);
    const token = await getOperationsToken();
    if (!token) return operationsResponse({ message: "Tu sesión expiró." }, 401);
    const { notificationId } = await context.params;
    return operationsResponse(await notificationsApi.delete(z.uuid().parse(notificationId), token));
  } catch (error) {
