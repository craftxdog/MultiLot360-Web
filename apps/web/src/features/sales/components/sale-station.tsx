...
import { canSellWithOwnAccount } from "../utils/sales-security";
...
export function SaleStation() {
  ...
  const sellerOptions = (sellers.data?.sellers ?? []).filter((seller) => seller.id !== user.data?.seller?.id);
  ...
  const canSellAsSelf = canSellWithOwnAccount(isAdmin, user.data?.seller?.id);
  const accountReady = canSellAsSelf;
  ...
  return <div className="space-y-4 pb-24 xl:pb-0">
    <section className="grid gap-3 rounded-2xl border border-border bg-card p-4 lg:grid-cols-2">
      ...
      {isAdmin ? <div>
        ...
        {canReadSellers ? <SalesSelect className="mt-2" ariaLabel="Vendedor" value={effectiveSellerId ?? ""} onChange={(value) => selectSeller(value || null)} options={[{ value: "", label: `Mi cuenta · ${user.data?.seller?.name ?? user.data?.username ?? "Administrador"}` }, ...sellerOptions.map((seller) => ({ value: seller.id, label: seller.name }))]} /> : <div className="mt-2 flex min-h-11 items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/8 px-4"><UserRoundCheck className="h-4 w-4 text-emerald-700 dark:text-emerald-300" /><div><p className="text-sm font-medium text-foreground">{user.data?.seller?.name ?? user.data?.username ?? "Administrador"}</p><p className="text-[10px] text-muted-foreground">Venta directa con tu cuenta administrativa.</p></div></div>}
        <p className="mt-2 text-[10px] text-muted-foreground">{effectiveSellerId ? "La venta se registrará representando al vendedor seleccionado." : "La venta quedará atribuida a tu propia membresía administrativa."}</p>
      </div> : <div>
    ...
    <div className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,.75fr)]">
      ...
      <aside className="sticky top-[4.5rem] hidden space-y-3 xl:block">
        <SaleCart />
        {!accountReady ? <div role="status" className="rounded-xl border border-danger/25 bg-danger/10 p-3 text-sm text-danger">Tu usuario no tiene un perfil de vendedor activo asociado.</div> : null}
