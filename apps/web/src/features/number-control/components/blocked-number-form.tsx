  ...
  return (
    <form className="space-y-6" onSubmit={form.handleSubmit(submit)}>
      ...
      <section className="rounded-2xl border border-border bg-muted/30 p-4">
        ...
        {scope === "DATE" ? <div className="mt-3"><Label htmlFor="blockDate">Fecha operativa</Label><Input id="blockDate" type="date" className="mt-2" {...form.register("date")} /><FieldError message={form.formState.errors.date?.message} /></div> : <div className="mt-3"><Label>Turno</Label>{canReadShifts ? <Controller control={form.control} name="shiftId" render={({ field }) => <NumberControlSelect className="mt-2" ariaLabel="Turno" value={field.value ?? ""} onChange={field.onChange} options={[{ label: "Selecciona un turno activo", value: "" }, ...(shifts.data?.shifts ?? []).map((shift) => ({ label: `${shift.configuration.code} · ${shift.date} ${shift.configuration.time.slice(0, 5)}`, value: shift.id }))]} />} /> : <p className="mt-3 rounded-xl border border-border bg-background p-3 text-xs text-muted-foreground">Necesitas permiso para listar turnos antes de bloquear un turno específico.</p>}<FieldError message={form.formState.errors.shiftId?.message} /></div>}
      </section>
