...
export function AuditWorkspace() {
  ...
  return <>
    <OperationsShell eyebrow="Trazabilidad segura" title="Auditoría" description="Consulta eventos HTTP y de dominio sin exponer contraseñas, códigos ni tokens." filters={filters}>
      {list.isLoading ? <LoadingRows /> : list.error ? <ErrorState message={list.error.message} /> : !list.data?.data.length ? <EmptyState>No hay eventos con estos filtros.</EmptyState> : <div className="space-y-2">{list.data.data.map((auditEvent) => <button key={auditEvent.id} type="button" onClick={() => setSelectedId(auditEvent.id)} className="grid w-full gap-2 rounded-xl border border-border p-4 text-left transition hover:bg-accent md:grid-cols-[1fr_auto_auto_24px] md:items-center"><span className="font-mono text-xs">{auditEvent.event}</span><span className="text-xs text-muted-foreground">{auditEvent.actor?.name ?? auditEvent.actor?.username ?? "Sistema"}</span><time className="text-xs text-muted-foreground">{formatOperationDateTime(auditEvent.createdAt)}</time><ChevronRight className="h-4 w-4 text-muted-foreground" /></button>)}</div>}
    ...
    <OperationDetailDrawer open={Boolean(selectedId)} eyebrow="Evento auditado" title={detail.data?.event ?? "Cargando evento…"} onClose={() => setSelectedId(null)}>
      {detail.error ? <ErrorState message={detail.error.message} /> : detail.isLoading ? <LoadingRows /> : detail.data ? <div className="space-y-4"><dl className="rounded-2xl border border-border bg-card p-4 text-sm"><dt className="text-xs text-muted-foreground">Actor</dt><dd className="mt-1">{detail.data.actor?.name ?? detail.data.actor?.username ?? "Sistema"}</dd><dt className="mt-4 text-xs text-muted-foreground">Fecha</dt><dd className="mt-1">{formatOperationDateTime(detail.data.createdAt)}</dd><dt className="mt-4 text-xs text-muted-foreground">ID</dt><dd className="mt-1 break-all font-mono text-xs">{detail.data.id}</dd></dl><pre className="max-h-[55vh] overflow-auto rounded-xl bg-muted p-4 text-[11px]">{JSON.stringify(detail.data.payload, null, 2)}</pre></div> : null}
