...
export function SalesMatrixWorkspace() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6">
    ...
    <section className="rounded-2xl border border-border bg-card p-4"><form className="grid gap-2 lg:grid-cols-[160px_170px_minmax(150px,1fr)_minmax(180px,1fr)_minmax(180px,1fr)_auto]" onSubmit={(event) => { event.preventDefault(); const values = new FormData(event.currentTarget); update({ date: String(values.get("date")), status: String(values.get("status")) as SalesMatrixStatus, drawCode: String(values.get("drawCode") || "").trim().toLowerCase() || undefined, shiftId: String(values.get("shiftId") || "").trim() || undefined, sellerId: String(values.get("sellerId") || "").trim() || undefined }); }}><Input name="date" type="date" defaultValue={query.date} aria-label="Fecha de la matriz" /><select name="status" defaultValue={query.status} aria-label="Estado de ventas" className="h-11 rounded-xl border border-border bg-background px-3 text-sm"><option value="ACTIVA">Activas</option><option value="ANULADA">Anuladas</option><option value="TODAS">Todas</option></select><Input name="drawCode" defaultValue={query.drawCode} placeholder="Código de sorteo" aria-label="Código de sorteo" /><Input name="shiftId" defaultValue={query.shiftId} placeholder="ID de turno" aria-label="ID de turno" /><Input name="sellerId" defaultValue={query.sellerId} placeholder="ID de vendedor" aria-label="ID de vendedor" /><Button type="submit" variant="secondary" className="h-11 gap-2"><Search className="h-4 w-4" />Filtrar</Button></form>
