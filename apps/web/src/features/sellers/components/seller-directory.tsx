...
export function SellerDirectory({ query }: { query: SellerDirectoryQuery }) {
  ...
  const canDeactivate = currentUser?.permissions.includes("vendedores.update") ?? false;
  const canDelete = currentUser?.permissions.includes("vendedores.delete") ?? false;
  ...
  const confirmAction = async () => {
    ...
    try {
      if (pendingAction.type === "deactivate") await deactivateSeller.mutateAsync(pendingAction.seller.id);
      else await deleteSeller.mutateAsync(pendingAction.seller.id);
  ...
  return <>
    ...
    {pendingAction ? <div className="fixed inset-0 z-[70] grid place-items-center bg-black/60 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && !mutating && setPendingAction(null)}><section role="alertdialog" aria-modal="true" aria-labelledby="seller-action-title" className="w-full max-w-md rounded-3xl border border-border bg-card p-6 shadow-2xl"><span className={`grid h-11 w-11 place-items-center rounded-2xl ${pendingAction.type === "delete" ? "bg-danger/10 text-danger" : "bg-amber-500/10 text-amber-700 dark:text-amber-300"}`}><AlertTriangle className="h-5 w-5" /></span><h2 id="seller-action-title" className="mt-4 text-lg font-semibold text-foreground">{pendingAction.type === "delete" ? "Eliminar vendedor definitivamente" : "Desactivar vendedor"}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{pendingAction.type === "delete" ? `Esta acción intentará eliminar por completo a ${pendingAction.seller.name}. Debe usarse sólo si la API confirma que no hay dependencias bloqueantes.` : `${pendingAction.seller.name} dejará de poder operar, pero su historial y auditoría se conservarán.`}</p><div className="mt-4 rounded-2xl border border-border bg-muted/30 p-3 text-xs text-muted-foreground"><p className="font-medium text-foreground">{pendingAction.seller.name}</p><p>@{pendingAction.seller.username} · {pendingAction.seller.documentId}</p></div><div className="mt-6 flex justify-end gap-2"><Button variant="ghost" onClick={() => setPendingAction(null)} disabled={mutating}>Cancelar</Button><Button variant={pendingAction.type === "delete" ? "danger" : "secondary"} onClick={confirmAction} disabled={mutating}>{mutating ? "Procesando…" : pendingAction.type === "delete" ? "Sí, eliminar" : "Sí, desactivar"}</Button></div></section></div> : null}
  </>;
}
